...
class HScript extends Iris
{
	...
	override function preset() {
		...
		set('SpriteFromSheet', objects.SpriteFromSheet);
		...
		set('Cursor', FlxG.mouse); set('Mouse', FlxG.mouse);

		set('Language', Language);
		set("Shader", Shader);
		set("ShaderFilter", ShaderFilter);
		set("GraphicsShader", GraphicsShader);
		set("FlxGraphicsShader", FlxGraphicsShader);
		...
		set('Map', StringMap); set('StringMap', StringMap);

		set('Json', Json); set('JsonHScript', Json); set('HScriptJson', Json);

		set('JsonParser', JsonParser); set('JsonParserHScript', JsonParser); set('HScriptJsonParser', JsonParser);

		set('TJson', TJson); set('TJsonHScript', TJson); set('HScriptTJson', TJson);

		set('MusicBeatState', backend.MusicBeatState);
		set('MusicBeatSubstate', backend.MusicBeatSubstate);

		set('play', PlayState.instance);

		set('loadLua', function(luaPath:String){
			new FunkinLua(Paths.getPath("scripts/" + luaPath));
		});
		...
		set("newShader", function(fragFile:String = null, vertFile:String = null) { // returns a FlxRuntimeShader but with file names lol
			...
			try
			{
				runtime = new flixel.addons.display.FlxRuntimeShader(fragFile == null ? null : NativeFileSystem.getContent(Paths.shaderFragment(fragFile)),
					vertFile == null ? null : NativeFileSystem.getContent(Paths.shaderVertex(vertFile)));
			}
			catch (e:Dynamic)
			{
