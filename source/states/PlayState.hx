...
class PlayState extends MusicBeatState
{
	...
	public static var hscriptExts = ['.hx', '.hxs', '.hxc', '.hxp', '.hxml', '.hscript'];
	...
	override public function create()
	{
		...
		#if HSCRIPT_ALLOWED for (ext in hscriptExts) startHScriptsNamed('stages/' + curStage + ext); #end
		...
		if (ClientPrefs.data.scoreTxtType == 'None'){
			scoreTxt = new FlxText(healthBar.x, healthBar.y + 30, FlxG.width, "", 20);
		...
		for (ext in hscriptExts){
			for (notetype in noteTypes)
				startHScriptsNamed('custom_notetypes/' + notetype + ext);
			for (event in eventsPushed)
				startHScriptsNamed('custom_events/' + event + ext);
		}
		#end
		noteTypes = null;
		eventsPushed = null;
		...
		for (i in luaArray)
			ModchartFuncs.loadLuaFunctions(i);
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true, behindUI:Bool = false)
	{
		#if VIDEOS_ALLOWED
		...
		if (foundFile)
		{
			videoCutscene = new VideoSprite(fileName, forMidSong, canSkip, loop, behindUI, this);
	...
	public dynamic function updateScoreText()
	{
		...
		if(totalPlayed != 0)
		{
			if (ClientPrefs.data.scoreTxtType == 'Psych 0.4.2'){
				percent = CoolUtil.floorDecimal(ratingPercent * 100, 0);
				str += ' (${percent}%)';
			}else{
	...
	override public function update(elapsed:Float)
	{
		...
		{
			if(ClientPrefs.data.timeBarType == 'Cool'){
				var curTime:Float = Math.max(0, Conductor.songPosition - ClientPrefs.data.noteOffset);
				songPercent = (curTime / songLength);

				var songCalc:Float = (songLength - curTime);
				var songCalc2:Float = curTime;
...
////////////////////////////////////////
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if(!note.hitCausesMiss) //Common notes
		{
			...
			else if (cpuControlled && note.isSustainNote) strumPlayAnim(false, Std.int(Math.abs(note.noteData)), Conductor.stepCrochet * 1.25 / 1000 / playbackRate);
			vocals.volume = 1;
