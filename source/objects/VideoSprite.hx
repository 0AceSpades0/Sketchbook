...
class VideoSprite extends FlxSpriteGroup {
	...
	public var behindUi:Bool = false;

	public function new(videoName:String, isWaiting:Bool, canSkip:Bool = false, shouldLoop:Dynamic = false, behindUi:Bool = false, game:PlayState) {
		super();
		...
		if (behindUi){
			videoSprite.cameras = [game.camHUD];
			game.addBehindNotes(videoSprite);
		}else{
			add(videoSprite);
		}
	...
	override function destroy()
	{
		...
		{
			if (behindUi){
				this.videoSprite.cameras = [];
				game.uiGroup.remove(this);
