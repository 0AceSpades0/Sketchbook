...
class VisualsSettingsSubState extends BaseOptionsMenu
{
	...
	public function new()
	{
		...
		var option:Option = new Option('Time Bar:',
			...
			'timeBarType',
			STRING,
			['Time Left', 'Time Elapsed', 'Song Name', 'Cool', 'Disabled']);
		addOption(option);
		...
		var option:Option = new Option('Squishy',
			"squish",
			'squish',
			BOOL);
		addOption(option);
		...
		var option:Option = new Option('Key Strokes',
			"enables a little thing in the corner that shows that key's you pressed!!",
			'keyStrokes',
			BOOL);
		addOption(option);
